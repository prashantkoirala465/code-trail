...
// The chain's whole point is that later rows lag more than earlier ones —
// one flat rate for every row makes the stack move as a single rigid
// object. Ramp it down the chain instead: fast at the head, slow at the
// tail, and the gradient is what reads as being dragged.
...
export function badgeCount(index: number, total: number): number {
  // A spindle, not a ramp that tops out and holds: 1 badge at the head,
  // up to MAX_BADGES through the belly, back down to 1 at the tail — a
  // flat run at the max reads as a rectangle sheared over instead of a
  // ribbon with a shape.
  const t = total <= 1 ? 0 : index / (total - 1);
  const spindle = Math.sin(Math.PI * t);
  return Math.max(
    1,
    Math.min(MAX_BADGES, 1 + Math.round(spindle * (MAX_BADGES - 1))),
  );
}
